/*
 * SPDX-License-Identifier: MIT
 *
 * Copyright (c) 2023 Bart Bilos
 * For conditions of distribution and use, see LICENSE file
 */
/**
 * @file LPC81X_syscon_hw.hpp
 * @brief Register structure and bit definitions for the LPC810 series System control
 */
#ifndef LPC81X_SYSCON_HW_HPP
#define LPC81X_SYSCON_HW_HPP

namespace libmcuhw::syscon {

/**
 * @brief sysctl register definitions
 */
struct Syscon {
  volatile std::uint32_t SYSMEMREMAP;    /*!<  System memory remap (R/W) */
  volatile std::uint32_t PRESETCTRL;     /*!<  Peripheral reset control (R/W) */
  volatile std::uint32_t SYSPLLCTRL;     /*!<  System PLL control (R/W) */
  volatile std::uint32_t SYSPLLSTAT;     /*!<  System PLL status (R/W ) */
  std::uint32_t RESERVED0[4];            /*!< reserved */
  volatile std::uint32_t SYSOSCCTRL;     /*!<  System oscillator control (R/W) */
  volatile std::uint32_t WDTOSCCTRL;     /*!< Watchdog oscillator control (R/W) */
  volatile std::uint32_t RESERVED1[2];   /*!< reserved */
  volatile std::uint32_t SYSRSTSTAT;     /*!< System reset status Register (R/W ) */
  std::uint32_t RESERVED2[3];            /*!< reserved */
  volatile std::uint32_t SYSPLLCLKSEL;   /*!< System PLL clock source select (R/W) */
  volatile std::uint32_t SYSPLLCLKUEN;   /*!< System PLL clock source update enable (R/W) */
  std::uint32_t RESERVED3[10];           /*!< reserved */
  volatile std::uint32_t MAINCLKSEL;     /*!< Main clock source select (R/W) */
  volatile std::uint32_t MAINCLKUEN;     /*!< Main clock source update enable (R/W) */
  volatile std::uint32_t SYSAHBCLKDIV;   /*!< System AHB clock divider (R/W) */
  std::uint32_t RESERVED4[1];            /*!< reserved */
  volatile std::uint32_t SYSAHBCLKCTRL;  /*!< System AHB clock control (R/W) */
  std::uint32_t RESERVED5[4];            /*!< reserved */
  volatile std::uint32_t UARTCLKDIV;     /*!< UART clock divider (R/W) */
  std::uint32_t RESERVED6[18];           /*!< reserved */
  volatile std::uint32_t CLKOUTSEL;      /*!< CLKOUT clock source select (R/W) */
  volatile std::uint32_t CLKOUTUEN;      /*!< CLKOUT clock source update enable (R/W) */
  volatile std::uint32_t CLKOUTDIV;      /*!< CLKOUT clock divider (R/W) */
  std::uint32_t RESERVED7;               /*!< reserved */
  volatile std::uint32_t UARTFRGDIV;     /*!< UART fractional divider SUB(R/W) */
  volatile std::uint32_t UARTFRGMULT;    /*!< UART fractional divider ADD(R/W) */
  std::uint32_t RESERVED8[1];            /*!< reserved */
  volatile std::uint32_t EXTTRACECMD;    /*!< External trace buffer command register  */
  volatile std::uint32_t PIOPORCAP0;     /*!< POR captured PIO status 0 (R/ ) */
  std::uint32_t RESERVED9[12];           /*!< reserved */
  volatile std::uint32_t IOCONCLKDIV[7]; /*!< Peripheral clock for IOCON block glitch filter */
  volatile std::uint32_t BODCTRL;        /*!< BOD control (R/W) */
  volatile std::uint32_t SYSTCKCAL;      /*!< System tick counter calibration (R/W) */
  std::uint32_t RESERVED10[6];           /*!< reserved */
  volatile std::uint32_t IRQLATENCY;     /*!< IRQ delay */
  volatile std::uint32_t NMISRC;         /*!< NMI Source Control     */
  volatile std::uint32_t PINTSEL[8];     /*!< GPIO Pin Interrupt Select register 0 */
  std::uint32_t RESERVED11[27];          /*!< reserved */
  volatile std::uint32_t STARTERP0;      /*!< Start logic signal enable Register 0 (R/W) */
  std::uint32_t RESERVED12[3];           /*!< reserved */
  volatile std::uint32_t STARTERP1;      /*!< Start logic signal enable Register 0 (R/W) */
  std::uint32_t RESERVED13[6];           /*!< reserved */
  volatile std::uint32_t PDSLEEPCFG;     /*!< Power-down states in Deep-sleep mode (R/W) */
  volatile std::uint32_t PDAWAKECFG;     /*!< Power-down states after wake-up (R/W) */
  volatile std::uint32_t PDRUNCFG;       /*!< Power-down configuration Register (R/W) */
  std::uint32_t RESERVED14[111];         /*!< reserved */
  volatile const std::uint32_t DEVICEID; /*!< Device ID (R/ ) */
};
namespace SYSMEMREMAP {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000003u}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kMAP_BOOT{0u << 0};          /*!< Map interrupts to boot ROM */
constexpr inline std::uint32_t kMAP_RAM{1u << 0};           /*!< Map interrupts to RAM */
constexpr inline std::uint32_t kMAP_FLASH{2u << 0};         /*!< Map interrupts to flash */
}  // namespace SYSMEMREMAP
namespace PRESETCTRL {
constexpr inline std::uint32_t kRESERVED_MASK{0x0001FFFFu}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kSPI0_RST_N{1u << 0};        /*!< SPI0 reset */
constexpr inline std::uint32_t kSPI1_RST_N{1u << 1};        /*!< SPI1 reset */
constexpr inline std::uint32_t kUARTFRG_RST_N{1u << 2};     /*!< UART FRG reset */
constexpr inline std::uint32_t kUART0_RST_N{1u << 3};       /*!< UART0 reset */
constexpr inline std::uint32_t kUART1_RST_N{1u << 4};       /*!< UART1 reset */
constexpr inline std::uint32_t kUART2_RST_N{1u << 5};       /*!< UART2 reset */
constexpr inline std::uint32_t kI2C_RST_N{1u << 6};         /*!< I2C reset */
constexpr inline std::uint32_t kMRT_RST_N{1u << 7};         /*!< MRT reset */
constexpr inline std::uint32_t kSCT_RST_N{1u << 8};         /*!< SCT reset */
constexpr inline std::uint32_t kWKT_RST_N{1u << 9};         /*!< WKT reset */
constexpr inline std::uint32_t kGPIO_RST_N{1u << 10};       /*!< GPIO reset */
constexpr inline std::uint32_t kFLASH_RST_N{1u << 11};      /*!< FLASH reset */
constexpr inline std::uint32_t kACMP_RST_N{1u << 12};       /*!< ACMP reset */
}  // namespace PRESETCTRL
namespace SYSPLLCTRL {
constexpr inline std::uint32_t kRESERVED_MASK{0x0000007Fu}; /*!< register mask for allowed bits */
/**
 * @brief Format feedback divider value
 *
 * @param divider feedback divider value, 0 is division by 1 to 31 is division by 32
 * @return formatted data for SYSPLLCTRL
 */
constexpr inline std::uint32_t MSEL(std::uint32_t divider) {
  return divider << 0;
}
constexpr inline std::uint32_t kPSEL_DIV2{0u << 5};  /*!< Post divider of 2 */
constexpr inline std::uint32_t kPSEL_DIV4{1u << 5};  /*!< Post divider of 4 */
constexpr inline std::uint32_t kPSEL_DIV8{2u << 5};  /*!< Post divider of 8 */
constexpr inline std::uint32_t kPSEL_DIV16{3u << 5}; /*!< Post divider of 16 */
}  // namespace SYSPLLCTRL
namespace SYSPLLSTAT {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000001u}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kLOCK{1u << 0};              /*!< PLL lock status */
}  // namespace SYSPLLSTAT
namespace SYSOSCCTRL {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000003u}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kNO_BYPASS{0u << 0};         /*!< PLL input is fed by crystal oscillator */
constexpr inline std::uint32_t kBYPASS{1u << 0};            /*!< PLL input is fed directly by XTALIN */
constexpr inline std::uint32_t kFREQ_1_20MHz{0u << 1};      /*!< crystal range is 1 to 20MHz */
constexpr inline std::uint32_t kFREQ_15_25MHz{1u << 1};     /*!< crystal range is 15 to 25MHz */
}  // namespace SYSOSCCTRL
namespace WDTOSCCTRL {
constexpr inline std::uint32_t kRESERVED_MASK{0x000001FFu}; /*!< register mask for allowed bits */
/**
 * @brief Format divider for Fclkana
 *
 * @param divider divider for Fclkana, 0 is 2, 1 is 4 and 31 is 64
 * @return formatted data for DIVSEL
 */
constexpr inline std::uint32_t DIVSEL(std::uint32_t divider) {
  return divider << 0;
}
constexpr inline std::uint32_t kFREQSEL_0_6MHZ{1 << 5};    /*!< Fclkana is 0.6 MHz */
constexpr inline std::uint32_t kFREQSEL_1_05MHZ{2u << 5};  /*!< Fclkana is 1.05 MHz */
constexpr inline std::uint32_t kFREQSEL_1_4MHZ{3u << 5};   /*!< Fclkana is 1.4 MHz */
constexpr inline std::uint32_t kFREQSEL_1_75MHZ{4u << 5};  /*!< Fclkana is 1.75 MHz */
constexpr inline std::uint32_t kFREQSEL_2_1MHZ{5u << 5};   /*!< Fclkana is 2.1 MHz */
constexpr inline std::uint32_t kFREQSEL_2_4MHZ{6u << 5};   /*!< Fclkana is 2.4 MHz */
constexpr inline std::uint32_t kFREQSEL_2_7MHZ{7u << 5};   /*!< Fclkana is 2.7 MHz */
constexpr inline std::uint32_t kFREQSEL_3_0MHZ{8u << 5};   /*!< Fclkana is 3.0 MHz */
constexpr inline std::uint32_t kFREQSEL_3_25MHZ{9u << 5};  /*!< Fclkana is 3.25 MHz */
constexpr inline std::uint32_t kFREQSEL_3_5MHZ{10u << 5};  /*!< Fclkana is 3.5 MHz */
constexpr inline std::uint32_t kFREQSEL_3_75MHZ{11u << 5}; /*!< Fclkana is 3.75 MHz */
constexpr inline std::uint32_t kFREQSEL_4_0MHZ{12u << 5};  /*!< Fclkana is 4.0 MHz */
constexpr inline std::uint32_t kFREQSEL_4_2MHZ{13u << 5};  /*!< Fclkana is 4.2 MHz */
constexpr inline std::uint32_t kFREQSEL_4_4MHZ{14u << 5};  /*!< Fclkana is 4.4 MHz */
constexpr inline std::uint32_t kFREQSEL_4_6MHZ{15u << 5};  /*!< Fclkana is 4.6 MHz */
}  // namespace WDTOSCCTRL
namespace SYSRSTSTAT {
constexpr inline std::uint32_t kRESERVED_MASK{0x0000001Fu}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kPOR{1u << 0};               /*!< POR detected */
constexpr inline std::uint32_t kEXTRST{1u << 1};            /*!< reset detected */
constexpr inline std::uint32_t kWDT{1u << 2};               /*!< WDT reset detected */
constexpr inline std::uint32_t kBOD{1u << 3};               /*!< BOD reset detected */
constexpr inline std::uint32_t kSYSRST{1u << 4};            /*!< System reset detected */
}  // namespace SYSRSTSTAT
namespace SYSPLLCLKSEL {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000003u}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kSEL_IRC{0u << 0};           /*!< PLL clock source is IRC */
constexpr inline std::uint32_t kSEL_SYSOSC{1u << 0};        /*!< PLL clock source is crystal oscillator */
constexpr inline std::uint32_t kSEL_CLKIN{3u << 0};         /*!< PLL clock source is external clock input */
}  // namespace SYSPLLCLKSEL
namespace SYSPLLCLKUEN {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000001u}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kNO_CHANGE{0u << 0};         /*!< No change of clock source */
constexpr inline std::uint32_t kUPDATE{1u << 0};            /*!< update clock source */
}  // namespace SYSPLLCLKUEN
namespace MAINCLKSEL {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000003u}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kSEL_IRC{0u << 0};           /*!< main clock source is IRC */
constexpr inline std::uint32_t kSEL_PLL_IN{1u << 0};        /*!< main clock source is PLL input */
constexpr inline std::uint32_t kSEL_WDTOSC{2u << 0};        /*!< main clock source is WDT oscillator */
constexpr inline std::uint32_t kSEL_PLL_OUT{3u << 0};       /*!< main clock source is PLL output */
}  // namespace MAINCLKSEL
namespace MAINCLKUEN {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000001u}; /*!< register mask for allowed bits */
constexpr inline std::uint32_t kNO_CHANGE{0u << 0};         /*!< No change of clock source */
constexpr inline std::uint32_t kUPDATE{1u << 0};            /*!< update clock source */
}  // namespace MAINCLKUEN
namespace SYSAHBCLKDIV {
constexpr inline std::uint32_t kRESERVED_MASK{0x000000FFu}; /*!< register mask for allowed bits */
/**
 * @brief Format system clock divider
 *
 * @param divider 0 is system clock disabled, 1 divide by 1, 255 divide by 255
 * @return formatted data for SYSAHBCLKDIV
 */
constexpr inline std::uint32_t DIV(std::uint32_t divider) {
  return divider << 0;
}
}  // namespace SYSAHBCLKDIV
namespace SYSAHBCLKCTRL {
constexpr inline std::uint32_t kRESERVED_MASK{0x000FFFFFu}; /*!< register mask for allowed bits */
}  // namespace SYSAHBCLKCTRL
namespace UARTCLKDIV {
constexpr inline std::uint32_t kRESERVED_MASK{0x000000FFu}; /*!< register mask for allowed bits */
}
namespace CLKOUTSEL {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace CLKOUTUEN {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace CLKOUTDIV {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace UARTFRGDIV {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace UARTFRGMULT {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace EXTTRACECMD {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace PIOPORCAP0 {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace IOCONCLKDIV {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace BODCTRL {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace SYSTCKCAL {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace IRQLATENCY {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace NMISRC {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace PINTSEL {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace STARTERP0 {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace STARTERP1 {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace PDSLEEPCFG {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace PDAWAKECFG {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
namespace PDRUNCFG {
constexpr inline std::uint32_t kRESERVED_MASK{0x000080EFu}; /*!< register mask for allowed bits */
}
namespace DEVICEID {
constexpr inline std::uint32_t kRESERVED_MASK{0x00000000u}; /*!< register mask for allowed bits */
}
}  // namespace libmcuhw::syscon
#endif