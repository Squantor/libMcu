...
struct Sct : libmcu::PeripheralBase {
  ...
  constexpr void Init(std::uint32_t prescale, CountingModes counting_mode) {
    ...
    if (counting_mode == CountingModes::BIDIRECTIONAL)
      ...
      sctPeripheral()->CTRL = hardware::CTRL::kHALT_L | hardware::CTRL::kCLRCTR_L | hardware::CTRL::PRE_L(prescale);
  }
  ...
  constexpr void SetupPwm(Matches match, std::uint32_t value, Events event, Outputs output, bool output_high) {
    ...
    sctPeripheral()->MATCH[match_index].U = value;
    sctPeripheral()->MATCHREL[match_index].U = value;
    sctPeripheral()->EV[event_index].CTRL = hardware::EV_CTRL::MATCHSEL(match_index) | hardware::EV_CTRL::kOUTSEL |
                                            hardware::EV_CTRL::IOSEL(output_index) | hardware::EV_CTRL::kCOMBMODE_MATCH;
    sctPeripheral()->EV[event_index].STATE = hardware::EV_STATE::kSTATEMASK0 | hardware::EV_STATE::kSTATEMASK1;
    ...
    sctPeripheral()->RES = hardware::RES::RES(sctPeripheral()->RES, output_index, hardware::RES::kTOGGLE);
  }
  /**
  ...
  constexpr void setupCapture(Captures capture, Events event, Inputs input, CaptureConditions condition) {
    ...
    sctPeripheral()->EV[eventIndex].CTRL = hardware::EV_CTRL::MATCHSEL(captureIndex) | hardware::EV_CTRL::kINSEL |
                                           hardware::EV_CTRL::IOSEL(inputIndex) | static_cast<std::uint32_t>(condition) |
                                           hardware::EV_CTRL::kCOMBMODE_IO;
    sctPeripheral()->EV[eventIndex].STATE = hardware::EV_STATE::kSTATEMASK0 | hardware::EV_STATE::kSTATEMASK1;
  }
  /**
