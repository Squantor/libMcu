...
typedef struct {
  ...
  /*  0x02: DMA mode, in case DMA block is available, DMA req function is called for UART DMA channel setup, then callback function
   * indicate that transfer completes */
...
typedef struct UARTD_API {
  ...
  UART_HANDLE_T *(*UartSetup)(
    uint32_t base_addr, uint8_t *ram); /* Setup Min UART instance with provided memory and return the handle to this instance */
  uint32_t (*UartInit)(UART_HANDLE_T *handle,
                       UART_CONFIG_T *set); /* Setup baud rate and operation mode for uart, then enable uart */

  /* UART polling functions block until completed */
  uint8_t (*UartGetChar)(UART_HANDLE_T *handle); /* Receive one Char from uart. This functions is only returned after Char is
                                                    received. In case Echo is enabled, the received data is sent out immediately */
  void (*UartPutChar)(UART_HANDLE_T *handle,
                      uint8_t data); /* Send one Char through uart. This function is only returned after data is sent */
  ...
  void (*UartIsr)(UART_HANDLE_T *handle); /* UART interrupt service routine. To use this routine, the corresponding USART interrupt
                                             must be enabled. This function is invoked by the user ISR */
} UARTD_API_T;
