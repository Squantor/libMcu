...
channels.
*/
static inline uint32_t DmaGetIntStatus(LPC_DMA_Type *pDMA) {
  return (pDMA->INTSTAT & ~DMA_INTSTAT_RESERVED);
}

/* DMA channel source/address/next descriptor */
typedef struct {
  uint32_t xfercfg; /* Transfer configuration (only used in linked lists and
                       ping-pong configs) */
...
*/
static inline void DmaSetSRAMBase(LPC_DMA_Type *pDMA, uint32_t base) {
  pDMA->SRAMBASE = (base & ~DMA_SRAMBASE_RESERVED);
}

static inline uint32_t DmaGetSRAMBase(LPC_DMA_Type *pDMA) {
  return (pDMA->SRAMBASE & ~DMA_SRAMBASE_RESERVED);
}

static inline void DmaEnableChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
  pDMA->DMACOMMON[0].ENABLESET = (1 << ch);
}

static inline void DmaDisableChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
...
*/
static inline uint32_t DmaGetEnabledChannels(LPC_DMA_Type *pDMA) {
...
*/
static inline uint32_t DmaGetActiveChannels(LPC_DMA_Type *pDMA) {
...
pipeline.
*/
static inline uint32_t DmaGetBusyChannels(LPC_DMA_Type *pDMA) {
...
*/
static inline uint32_t DmaGetErrorIntChannels(LPC_DMA_Type *pDMA) {
  return (pDMA->DMACOMMON[0].ERRINT & ~DMA_ERRINT_RESERVED);
}

static inline void DmaClearErrorIntChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
  pDMA->DMACOMMON[0].ERRINT = (1 << ch);
}

static inline void DmaEnableIntChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
  pDMA->DMACOMMON[0].INTENSET = (1 << ch);
}

static inline void DmaDMA_DisableIntChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
...
*/
static inline uint32_t DmaGetEnableIntChannels(LPC_DMA_Type *pDMA) {
...
*/
static inline uint32_t DmaGetActiveIntAChannels(LPC_DMA_Type *pDMA) {
...
*/
static inline uint32_t DmaGetActiveIntBChannels(LPC_DMA_Type *pDMA) {
...
static inline void DmaSetValidChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
  pDMA->DMACOMMON[0].SETVALID = (1 << ch);
}

static inline void Chip_DMA_SetTrigChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
...
again.
*/
static inline void DmaAbortChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
...
#define DMA_CFG_BURSTPOWER_16                    \
  (4 << 8) /* Set DMA burst size to 16 transfers \
            */
...
static inline uint32_t DmaGetChannelStatus(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
...
static inline void DmaSetChannelInValid(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
  DmaClearTranBits(pDMA, ch, DMA_XFERCFG_CFGVALID);
}

static inline void DmaSWTriggerChannel(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
  DmaSetTranBits(pDMA, ch, DMA_XFERCFG_SWTRIG);
}

static inline bool DmaIsChannelActive(LPC_DMA_Type *pDMA, DMA_CHID_T ch) {
