...
typedef volatile struct { /* ADCn Structure */
  uint32_t CTRL;          /* A/D Control Register. The AD0CR register must be written to
  ...
  uint32_t SEQ_GDAT[ADC_SEQB_IDX + 1]; /* A/D Sequence A & B Global Data Register. Contains the
                                          result of the most recent A/D conversion for sequence. */
  const uint32_t RESERVED1[2];
  const uint32_t DR[12]; /* A/D Channel Data Register. This register contains the result of
