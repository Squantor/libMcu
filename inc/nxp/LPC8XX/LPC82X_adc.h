...
typedef enum { ADC_SEQA_IDX, ADC_SEQB_IDX } ADC_SEQ_IDX_T;

typedef volatile struct { /* ADCn Structure */
  uint32_t
      CTRL; /* A/D Control Register. The AD0CR register must be written to
               select the operating mode before A/D conversion can occur. */
  const uint32_t RESERVED0;
  uint32_t SEQ_CTRL[ADC_SEQB_IDX + 1]; /* A/D Sequence A & B Control Register.
                                          Controls triggering and channel
                                          selection for sonversion sequence. */
  uint32_t
      SEQ_GDAT[ADC_SEQB_IDX +
               1]; /* A/D Sequence A & B Global Data Register. Contains the
                      result of the most recent A/D conversion for sequence. */
  const uint32_t RESERVED1[2];
  const uint32_t
      DR[12]; /* A/D Channel Data Register. This register contains the result of
                 the most recent conversion completed on channel n. */
  uint32_t THR_LOW[2]; /* A/D Low Compare Threshold Register 0 & 1. Contains the
                          lower threshold level for automatic threshold
                          comparison. */
  uint32_t THR_HIGH[2]; /* A/D High Compare Threshold Register 0 & 1. Contains
                           the higher threshold level for automatic threshold
                           comparison. */
  uint32_t CHAN_THRSEL; /* A/D Channel Threshold Select Register. Specifies
                           which set of threshold compare registers to use. */
  uint32_t INTEN;       /* A/D Interrupt Enable Register. This register contains
                           enable bits that enable sequence-A, sequence-B, threshold
                           compare and overrun interrupts. */
  uint32_t FLAGS; /* A/D Flags Register. This register contains interrupt flags.
                     - To be checked */
...
#define ADC_CR_CLKDIV(n)                                                       \
  ((((n)&0xFF) << 0)) /**< The APB clock (PCLK) is divided by (this value plus \
                         one) to produce the clock for the A/D */
#define ADC_CR_ASYNMODE \
  (1 << 8) /**< Asynchronous mode enable bit, undocumented in datasheet */
...
#define ADC_SEQ_CTRL_HWTRIG_HIGH \
  (0 << 12) /**< HW trigger input - Always logic high */
#define ADC_SEQ_CTRL_HWTRIG_PINTRIG0 \
  (1 << 12) /**< HW trigger input - ADC pin trigger 0, connect via SWM */
#define ADC_SEQ_CTRL_HWTRIG_PINTRIG1 \
  (2 << 12) /**< HW trigger input - ADC pin trigger 1, connect via SWM */
#define ADC_SEQ_CTRL_HWTRIG_SCT0_OUT3 \
  (3 << 12) /**< HW trigger input - SCT0 output 3 */
#define ADC_SEQ_CTRL_HWTRIG_ACMP_O \
  (4 << 12) /**< HW trigger input - Analog comparator output */
#define ADC_SEQ_CTRL_HWTRIG_ARM_TXEV \
  (5 << 12) /**< HW trigger input - ARM core TXEV event */
#define ADC_SEQ_CTRL_HWTRIG_MASK \
  (0x7 << 12) /**< HW trigger input bitfield mask */

#define ADC_SEQ_CTRL_HWTRIG_POLPOS \
  (1 << 18) /* HW trigger polarity - positive edge */
#define ADC_SEQ_CTRL_HWTRIG_SYNCBYPASS \
  (1 << 19)                          /* HW trigger bypass synchronisation */
...
#define ADC_SEQ_CTRL_LOWPRIO \
  (1 << 29) /* High priority enable bit (regardless of name) */
...
#define ADC_DR_CHANNEL(n) \
  (((n) >> ADC_DR_CHAN_BITPOS) & 0xF) /* Channel number bit position */
...
#define ADC_THRSEL_CHAN_SEL_THR1(n) \
  (1 << (n)) /* Select THR1 register for channel n */
...
#define ADC_INTEN_SEQN_ENABLE(seq) \
  (1 << (seq))                           /* Sequence A/B Interrupt enable bit */
...
#define ADC_INTEN_CMP_ENABLE(isel, ch) \
  (((isel)&ADC_INTEN_CMP_MASK)         \
   << ((2 * (ch)) + 3)) /* Interrupt selection for channel */

#define ADC_FLAGS_THCMP_MASK(ch) \
  (1 << (ch)) /* Threshold comparison status for channel */
#define ADC_FLAGS_OVRRUN_MASK(ch) \
  (1 << (12 + (ch)))                         /* Overrun status for channel */
#define ADC_FLAGS_SEQA_OVRRUN_MASK (1 << 24) /* Seq A Overrun status */
#define ADC_FLAGS_SEQB_OVRRUN_MASK (1 << 25) /* Seq B Overrun status */
#define ADC_FLAGS_SEQN_OVRRUN_MASK(seq) \
  (1 << (24 + (seq)))                     /* Seq A/B Overrun status */
#define ADC_FLAGS_SEQA_INT_MASK (1 << 28) /* Seq A Interrupt status */
#define ADC_FLAGS_SEQB_INT_MASK (1 << 29) /* Seq B Interrupt status */
#define ADC_FLAGS_SEQN_INT_MASK(seq) \
  (1 << (28 + (seq))) /* Seq A/B Interrupt status */
#define ADC_FLAGS_THCMP_INT_MASK \
  (1 << 30) /* Threshold comparison Interrupt status */
#define ADC_FLAGS_OVRRUN_INT_MASK (1UL << 31) /* Overrun Interrupt status */

#define ADC_TRIM_VRANGE_HIGHV \
  (0 << 5) /* Voltage range bit - High volatge (2.7V to 3.6V) */
#define ADC_TRIM_VRANGE_LOWV \
  (1 << 5) /* Voltage range bit - Low volatge (1.8V to 2.7V) */
...
*/
static inline void AdcSetSequencerBits(LPC_ADC_T *pADC, ADC_SEQ_IDX_T seqIndex,
                                       uint32_t bits) {
  pADC->SEQ_CTRL[seqIndex] =
      (pADC->SEQ_CTRL[seqIndex] & ~ADC_SEQ_CTRL_RES) | bits;
}
...
while maintaining bits 20..25 as 0, regardless of the read state of those bits.
*/
static inline void AdcClearSequencerBits(LPC_ADC_T *pADC,
...
options are passed to the functions as a OR'ed list of values. This function
will disable/clear the sequencer start/burst/single step/enable if they are
enabled.<br> Select the channels by OR'ing in one or more
ADC_SEQ_CTRL_CHANSEL(ch) values.<br> Select the hardware trigger by OR'ing in
one ADC_SEQ_CTRL_HWTRIG_* value.<br> Select a positive edge hardware trigger by
OR'ing in ADC_SEQ_CTRL_HWTRIG_POLPOS.<br> Select trigger bypass synchronisation
by OR'ing in ADC_SEQ_CTRL_HWTRIG_SYNCBYPASS.<br> Select ADC single step on
trigger/start by OR'ing in ADC_SEQ_CTRL_SINGLESTEP.<br> Select higher priority
conversion on the other sequencer by OR'ing in ADC_SEQ_CTRL_LOWPRIO.<br> Select
end of seqeuence instead of end of conversion interrupt by OR'ing in
ADC_SEQ_CTRL_MODE_EOS.<br> Example for setting up sequencer A (channels 0-2,
trigger on high edge of PIO0_2, interrupt on end of sequence):<br>
ADC_SetupSequencer(LPC_ADC, ADC_SEQA_IDX, (
    ADC_SEQ_CTRL_CHANSEL(0) | ADC_SEQ_CTRL_CHANSEL(1) | ADC_SEQ_CTRL_CHANSEL(2)
| ADC_SEQ_CTRL_HWTRIG_PIO0_2 | ADC_SEQ_CTRL_HWTRIG_POLPOS |
ADC_SEQ_CTRL_MODE_EOS));
*/
static inline void AdcSetupSequencer(LPC_ADC_T *pADC, ADC_SEQ_IDX_T seqIndex,
                                     uint32_t options) {
...
static inline void AdcDisableSequencer(LPC_ADC_T *pADC,
                                       ADC_SEQ_IDX_T seqIndex) {
...
ADC burst sequence.
*/
static inline void AdcStartBurstSequencer(LPC_ADC_T *pADC,
                                          ADC_SEQ_IDX_T seqIndex) {
  AdcSetSequencerBits(pADC, seqIndex, ADC_SEQ_CTRL_BURST);
}

static inline void AdcStopBurstSequencer(LPC_ADC_T *pADC,
                                         ADC_SEQ_IDX_T seqIndex) {
...
uint32_t adcDataRawValue = ADC_ReadSequencerDataReg(LPC_ADC, ADC_SEQA_IDX); //
Get raw value uint32_t adcDataValue = ADC_DR_RESULT(adcDataRawValue); // Aligned
and masked ADC data value ADC_DR_THCMPRANGE_T adcRange = (ADC_DR_THCMPRANGE_T)
ADC_DR_THCMPRANGE(adcDataRawValue); // Sample range compared to threshold
low/high ADC_DR_THCMPCROSS_T adcRange = (ADC_DR_THCMPCROSS_T)
ADC_DR_THCMPCROSS(adcDataRawValue); // Sample cross compared to threshold low
uint32_t channel = ADC_DR_CHANNEL(adcDataRawValue); // ADC channel for this
sample/data bool adcDataOverrun = (bool) ((adcDataRawValue & ADC_DR_OVERRUN) !=
0); // Data overrun flag bool adcDataValid = (bool) ((adcDataRawValue &
ADC_SEQ_GDAT_DATAVALID) != 0); // Data valid flag
*/
static inline uint32_t AdcGetSequencerDataReg(LPC_ADC_T *pADC,
                                              ADC_SEQ_IDX_T seqIndex) {
...
static inline void AdcSetThrLowValue(LPC_ADC_T *pADC, uint8_t thrnum,
                                     uint16_t value) {
  pADC->THR_LOW[thrnum] = (((uint32_t)value) << ADC_THR_VAL_POS);
}

static inline void AdcSetThrHighValue(LPC_ADC_T *pADC, uint8_t thrnum,
                                      uint16_t value) {
...
Example:
ADC_SelectTH0Channels(LPC_ADC, (ADC_THRSEL_CHAN_SEL_THR1(1) |
ADC_THRSEL_CHAN_SEL_THR1(2))); // Selects channels 1 and 2 for threshold 0
*/
...
Example:
ADC_SelectTH1Channels(LPC_ADC, (ADC_THRSEL_CHAN_SEL_THR1(4) |
ADC_THRSEL_CHAN_SEL_THR1(5))); // Selects channels 4 and 5 for 1 threshold
*/
...
static inline void AdcSetThresholdInt(LPC_ADC_T *pADC, uint8_t ch,
                                      ADC_INTEN_THCMP_T thInt) {
  pADC->INTEN = (pADC->INTEN & ~(ADC_INTEN_RES | (3 << (3 + (ch * 2))))) |
                (thInt << (3 + (ch * 2)));
}
...
Example:
if (ADC_GetFlags(LPC_ADC) & ADC_FLAGS_THCMP_MASK(3) // Check of threshold comp
status for ADC channel 3
*/
static inline uint32_t AdcGetFlags(LPC_ADC_T *pADC) { return pADC->FLAGS; }
